...
function accountswitcher_usercp()
{
    ...
    if ($mybb->input['action'] == 'as_registerattach') {
        if ($mybb->usergroup['as_canswitch'] == 1 || $mybb->user['as_uid'] != 0 || $mybb->user['as_share'] != 0) {
            if (isset($mybb->settings['aj_regmailattach']) && $mybb->settings['aj_regmailattach'] == 1) {
                ...
                if (count($masters) == 0) {
                    ...
                    if (count($masters) == 1) {
                        $master_acc = get_user((int)$masters[0]);
                        $selectoptions .= '
...
//########## ACTIONS ##########
    ...
    if ($mybb->input['action'] == 'as_attach'
        ...
        // Get the target
        $target = get_user_by_username($mybb->get_input('username'), array('fields' => '*'));

        // User exist? Password correct?
        if (!$target) {
            error($lang->as_invaliduser);
        }
        if (validate_password_from_uid($target['uid'], $mybb->get_input('password')) == false) {
            error($lang->as_invaliduser);
        }

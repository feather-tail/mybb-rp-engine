...
function accountswitcher_header()
{
    ...
    global $as_header_userbit, $as_sidebar, $as_sidebar_userbit, $as_header_dropdown, $as_post, $access;
    global $as_post_userbit, $pm_switch_notice, $privatemessage_bit, $privatemessage_switch, $mybbUsername;
    global $mybbUid, $attachedPostUser, $userUid, $attachedUser, $userAvatar, $user_profilefield, $ownAvatar;
    ...
    if ($mybb->user['uid'] != 0) {
		...
		$access = (bool)true;
		if (isset($mybb->settings['aj_forum_perm']) && $mybb->settings['aj_forum_perm'] == 1) {
			$access = false;
		}
        ...
        if ($mybb->usergroup['as_canswitch'] == 1 && $count > 0) {
            ...
            // Sort accounts by first, secondary, shared accounts and by uid or username
                ...
                foreach ($accounts as $key => $account) {
					// Check if account has permission for viewing the forum/thread
					if ($access === false) { 
						$permission = accountswitcher_forumpermissions($account['uid']);
						if ($permission !== true) {
							continue;
						}
					}
                    ...
                    if ($account['as_uid'] == $mybb->user['uid']) {
                        if ($count > 0) {
                            ...
                            if ($mybb->settings['aj_pmnotice'] == 1
                                ...
                                && ($current_page != "private.php"
                                    || (isset($mybb->input['action']) && $mybb->input['action'] != "read"))
                            ) {
                ...
                if (!empty(trim($as_header_userbit ?? ''))) {
        ...
        if ($count == 0
            && empty($mybb->user['as_uid'])
            && empty($mybb->user['as_share'])
        ...
        if ($mybb->settings['aj_shareuser'] == 1
            ...
            if ($access === false) {
				$permission = accountswitcher_forumpermissions($userUid);
			}
            if ($mybb->settings['aj_postjump'] == 1 && $permission !== false) {
                $as_post_userbit .= eval($templates->render('accountswitcher_post_userbit'));
            }
    ...
    if (isset($mybb->settings['aj_headerdropdown'])
        ...
        && !empty(trim($as_header_dropdown ?? ''))
    ) {
        $as_header = eval($templates->render('accountswitcher_header_dropdown'));
    // Default header
    } elseif (!empty(trim($as_header_userbit ?? ''))) {
        $as_header = eval($templates->render('accountswitcher_header'));
    }
}
...
function accountswitcher_author()
{
    ...
    global $userUid, $attachedUser, $as_author_userbit, $cancel, $access;

    // If user author change or mod author change
    if ((isset($mybb->input['changeauthor'])
            && $mybb->input['changeauthor'] == 1
            && $mybb->settings['aj_changeauthor'] == 1)
        || (isset($mybb->input['adminauthor'])
            && $mybb->input['adminauthor'] == 1
            ...
            // Change author to attached user
                ...
                if ($mybb->usergroup['as_canswitch'] == 1 && $count > 0) {
                    ...
                    // Sort accounts by first, secondary, shared accounts and by uid or username
                        ...
                        foreach ($accounts as $key => $account) {
							if ($access === false) {
								$fpermission = accountswitcher_forumpermissions($account['uid']);
								if ($fpermission !== true) {
									continue;
								}
							}
...
function accountswitcher_author_change()
{
    ...
    // Change action
    if (isset($mybb->input['action']) && $mybb->input['action'] == 'do_author'
...
$plugins->add_hook('postbit_prev', 'accountswitcher_author_change_button', 50);
$plugins->add_hook('postbit_pm', 'accountswitcher_author_change_button', 50);
$plugins->add_hook('postbit_announcement', 'accountswitcher_author_change_button', 50);
/**
...
{
    global $mybb, $theme, $lang, $admin_change, $author_change, $templates, $eas, $thread;
    ...
    if (($mybb->user['uid'] != 0
            ...
            && ($count > 0 || !empty($mybb->user['as_uid']))
            && $mybb->settings['aj_changeauthor'] == 1
        )
        || ($mybb->settings['aj_admin_changeauthor'] == 1
...
function accountswitcher_profile()
{
    ...
    // Would we like to have a profile field displayed?
    // 1. Change $as_field from false to true
    // 2. Use {$as_profilefield} in templates "accountswitcher_profile_link" and "accountswitcher_profile_switch"
    $as_field = (bool)false;
    ...
    // Get usergroup permissions
        ...
        $profile_attached = $as_profile_userbit = $as_profile_hidden = $as_profilefield = '';
        ...
        if ($permissions['as_canswitch'] == 1 && $count > 0) {
            ...
            $as_profile_userbit = $eas->master_userlist($memprofile['uid'], $as_field);
        }
            ...
            if ($permission['as_canswitch'] == 1) {
                ...
                if ($memprofile['uid'] == $mybb->user['uid']) {
                    $attachedUser = '<span style="font-weight: bold;" title="Master Account">'.
                                    ...
                                    .'</span>';
                    if ($as_field === true) {
                        $as_profilefield .= $eas->get_profilefield($masterUid);
                    }
                ...
                // Get all users attached to master from the cache
                $as_profile_userbit .= $eas->attached_userlist($memprofile['uid'], $as_field);
            }
        }
        ...
        if ($count > 0 || $count == 0 && $memprofile['as_uid'] != 0) {
            ...
            if ($memprofile['uid'] == $mybb->user['uid']
                || (!empty($mybb->user['as_uid']) && $memprofile['uid'] == $mybb->user['as_uid'])
...
$plugins->add_hook('postbit_prev', 'accountswitcher_postuser');
$plugins->add_hook('postbit_pm', 'accountswitcher_postuser');
$plugins->add_hook('postbit_announcement', 'accountswitcher_postuser');
/**
...
function accountswitcher_postuser(&$post)
{
    ...
    global $as_postcount, $user_profilefield, $templates, $postId;

    $post['attached_accounts'] = "";
    // Get the permission to view the list, super admins are always allowed
    if (isset($mybb->settings['aj_groupperm'])
        && $mybb->settings['aj_groupperm'] != -1
        && !is_super_admin($mybb->user['uid'])
    ...
    // Get usergroup permissions
        ...
        if ($count > 0 || $count == 0 && $post['as_uid'] != 0) {
            ...
            // Show hidden accounts if current user is post author or attached to hidden account
            if ($post['uid'] == $mybb->user['uid']
                || (!empty($mybb->user['as_uid']) && $post['uid'] == $mybb->user['as_uid'])
...
function accountswitcher_memberlist(&$user)
{
    ...
    // Get usergroup permissions
        ...
        if ($count > 0 || $count == 0 && $user['as_uid'] != 0) {
            ...
            // Show hidden accounts if current user is post author or attached to hidden account
            if ($user['uid'] == $mybb->user['uid']
                || (!empty($mybb->user['as_uid']) && $user['uid'] == $mybb->user['as_uid'])
...
function accountswitcher_clear_shared()
{
    ...
    if (isset($shareduser['uid']) && $shareduser['uid'] != 0) {
...
function accountswitcher_register_attach()
{
    ...
    if (isset($mybb->settings['aj_regmailattach']) && $mybb->settings['aj_regmailattach'] == 1) {
        ...
        if (count($masters) > 0) {
            ...
            // Is the current user group allowed to use the account switcher
            $group_perms = usergroup_permissions($user_info['usergroup']);
            if ($group_perms['as_canswitch'] == 1) {
...
function accountswitcher_agreement_save()
{
    ...
    } elseif ($count == 0 && !empty($mybb->user['as_uid']) && is_array($accounts)) {
...
// Hook for displaying list on index
$plugins->add_hook('index_start', 'accountswitcher_indexlist');
/**
 * Adds a list of top master accounts on index
 *
 */
function accountswitcher_indexlist()
{
    global $mybb, $eas, $masters_toplist;

    // Load account data from cache
    $accounts = $eas->accountswitcher_cache;
    $masters_uid = $masters_count = array();
    ...
    if (is_array($accounts)) {
        ...
        foreach ($masters_uid as $master_uid) {
            ...
            $masters_count[$master_name] = (int)str_replace($mybb->settings['thousandssep'], '', $master['postnum']);
            $masters_count[$master_name] += $eas->get_postcount($master_uid);
        }
    }
    arsort($masters_count);

    $masters_toplist = "<p>Master: - Attached posts:</p><p>";

    $i = 1;
    foreach ($masters_count as $x => $x_value) {
        if ($i <= 5) {
            $masters_toplist .= $x."  -  ".$x_value."<br />";
        }
        ++$i;
    }
...
// Check if account has permission for viewing the forum/thread 
function accountswitcher_forumpermissions($uid=0)
{
	global $mybb;
	...
	switch (THIS_SCRIPT) {
		...
		case "newthread.php":
			...
			$forumpermissions = forum_permissions($fid, $uid);

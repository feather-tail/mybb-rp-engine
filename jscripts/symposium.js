(function () {
	...
	Symposium = {
		...
		bindUI: function () {
			...
			$(document)
				...
				.on('change.symposium', '[data-message-id].delete', function () {
					$(this).closest('[id^="pm-"], [id*="pm-"]').toggleClass('highlighted');
				});
		},
		...
		getLastPmid: function () {
			var last = $('#conversationContainer').find('[id^="pm-"], [id*="pm-"]').last();
			if (!last.length) {
				return 0;
			}
		...
		scrollToBottom: function (force) {
			...
			var nearBottom = (d.prop('scrollHeight') - d.scrollTop() - d.outerHeight()) < 160;
		...
		appendMessagesHtml: function (html) {
			...
			var wasAtBottom = (container.prop('scrollHeight') - container.scrollTop() - container.outerHeight()) < 160;
		...
		sendMessage: function (form) {
			...
			return Symposium.ajax.request(data, 'POST', (response) => {
				...
				if (response && response.success) {
					...
					Symposium.numberOfMessages = Math.max(1, (Symposium.numberOfMessages || 0) + 1);
					Symposium.poll(true);
				}
			});
		},
				...
				return $.ajax({
					...
					complete: (xhr) => {
						...
						try {
							...
							console.log(e);
							$.jGrowl(Symposium.lang.responseMalformed, { theme: 'jgrowl_error' });
							return;
						}
